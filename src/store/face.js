                ...
                const labeledDescriptors = await Promise.all(
                    state.faces.flatMap(async (face, index) => {
                        ...
                        const descriptions = await Promise.all(images.map(async image => {
                            ...
                            const detections = await faceapi.detectSingleFace(img, getters.options).withFaceLandmarks(state.model == 'tiny').withFaceDescriptor()
                        ...
                        return new faceapi.LabeledFaceDescriptors(`${index}`, descriptionsFilter)
                    })
                );
