  ...
  <v-container>
    <v-row>
      <v-col cols="12" class="py-0">
        <h1 class="font-weight-light mb-0">Tabel Data Penguji</h1>
      </v-col>
      <v-col cols="12">
        <Table
          @tambah="tambah"
...
export default {
  ...
  computed: {
    ...mapState({
      items: (state) => state.pengujiModule.pengujis,
    }),
    formTitle() {
    ...
    nbmRules() {
      return [
        (v) => !!v || "NBM tidak boleh kosong",
        (v) => {
          return (
            (this.editedIndex != -1 && this.items[this.editedIndex].nbm == v) ||
            !this.items.find((item) => item.nbm == v) ||
            "NBM telah digunakan"
          );
        },
      ];
  ...
  methods: {
    ...
    async hapus() {
      const res = await this.deletePenguji({
    ...
    async simpan() {
      ...
      if (this.editedIndex > -1) {
        res = await this.editPenguji({
          index: this.editedIndex,
          penguji: { ...this.editedItem },
        });
      } else {
        res = await this.addPenguji({ ...this.editedItem });
      }
