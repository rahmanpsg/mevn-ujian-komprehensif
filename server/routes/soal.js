...
itemRouter.get("/", async (req, res) => {
  ...
  soals.forEach((v) => {
    ...
    listSoalByMatakuliah[v.matakuliah.matakuliah] = listSoalByMatakuliah[
      v.matakuliah.matakuliah
    ]
      ? [...listSoalByMatakuliah[v.matakuliah.matakuliah], v]
      : [v];
  });
  ...
  const maxSoalPerMatakuliah = 1;
  for (const [mk, soal] of Object.entries(listSoalByMatakuliah)) {
    for (
      let i = maxSoalPerMatakuliah * 2 - soal.length;
      i <= soal.length - 1;
      i++
    ) {
      listSoalByMatakuliah[mk].splice(
